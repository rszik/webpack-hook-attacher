...
class WebpackHookAttacherPlugin {
    ...
    apply(compilerAnyInstance) {
        ...
        this.initHookGroups(this.options.compilerHooks);
        this.initHookGroups(this.options.javascriptParserHooks);
        this.initHookGroups(this.options.compilationHooks);
        this.initHookGroups(this.options.normalModuleFactoryHooks);
        this.initHookGroups(this.options.contextModuleFactoryHooks);
    }
    ...
    getTapable(hook, operation = null) {
        ...
        else if (hook.hookType === operation_1.HookType.NormalModuleFactoryHook) {
            return this.compilerInstance;
        }
        else if (hook.hookType === operation_1.HookType.ContextModuleFactoryHook) {
            return this.compilerInstance;
        }
    }
    ...
    setOperationTapType(hook, operation) {
        switch (hook.webpackHookType) {
            ...
            case options_1.WebpackHookType.HookMap:
            case options_1.WebpackHookType.HookMap_SyncHook:
            case options_1.WebpackHookType.HookMap_SyncBailHook:
            ...
            case options_1.WebpackHookType.AsyncSeriesWaterfallHook:
                if (operation.params.asyncDefaultTapType !== operation_base_classes_1.TapType.NotSet) {
        ...
        hook.callbackParameterNames.forEach((hookTypeParameterName, index) => {
            switch (hook.hookType) {
                ...
                case operation_1.HookType.JavascriptParserHook:
                    ...
                    break;
                case operation_1.HookType.ContextModuleFactoryHook:
                    operation.contextModuleFactoryHookCallbackParameters[hookTypeParameterName] = params[index];
                    break;
                case operation_1.HookType.NormalModuleFactoryHook:
                    operation.normalModuleFactoryHookCallbackParameters[hookTypeParameterName] = params[index];
                    break;
                default:
